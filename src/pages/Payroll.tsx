...
export default function Payroll() {
  ...
  const { data: invoices, isLoading } = useQuery({
    ...
    queryFn: async () => {
      // Load initial page only; prevents huge payloads from stalling the UI.
      let q = supabase
        .from("payroll_invoices")
        .select(
          "id, invoice_number, staff_id, payroll_type, period_start, period_end, gross_amount, deductions, net_amount, due_date, issued_date, status, notes"
        )
  ...
  const staffIdsInInvoices = useMemo(() => {
    const ids = (invoices ?? [])
      .map((inv: any) => inv?.staff_id)
      .filter(Boolean);
    return Array.from(new Set(ids));
  }, [invoices]);

  const { data: staffById } = useQuery({
    queryKey: ["staff-by-id", staffIdsInInvoices],
    enabled: isManager && staffIdsInInvoices.length > 0,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("staff")
        .select("id, full_name")
        .in("id", staffIdsInInvoices);
      if (error) throw error;
      return data as { id: string; full_name: string }[];
    },
  });
  ...
  const staffNameById = useMemo(() => {
    ...
    for (const s of staffById ?? []) map.set(s.id, s.full_name);
    return map;
  }, [staffById]);
